...
const resolvers = {
  ...
  Mutation: {
    ...
    removeOwner: (root, args) => {
      const removedOwner = find(owner, { id: args.id })
      if (!removedOwner) {
        throw new Error(`Couldn't find owner with id ${args.id}`)
      }
      remove(owner, c => { return c.id === removedOwner.id })
      return removedOwner
    },
    addCar: (root, args) => {
      const newCar = {
    ...
    removeCar: (root, args) => {
      const removedCar = find(car, { id: args.id })
      if (!removedCar) {
        throw new Error(`Couldn't find owner with id ${args.id}`)
      }
      remove(car, c => { return c.id === removedcar.id })
      return removedcar
    }
  }
}
